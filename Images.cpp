...
static void
png_write_aopt_file(png_structp png_ptr, png_bytep data, png_size_t length)
{
    AoptFile* aoptfile = (AoptFile*) png_get_io_ptr(png_ptr);
    status_t err = aoptfile->writeData(data, length);
    if (err != NO_ERROR) {
...
static void
png_flush_aopt_file(png_structp /* png_ptr */)
{
}
                      ...
                      image_info& imageInfo, const Bundle* bundle)
{
    ...
    // Legacy versions of aopt would always encode 9patch PNGs as RGBA.  This had the unintended
...
static bool read_png_protected(png_structp read_ptr, String8& printableName, png_infop read_info,
                               const sp<AoptFile>& file, FILE* fp, image_info* imageInfo) {
...
status_t preProcessImage(const Bundle* bundle, const sp<AoptAssets>& /* assets */,
                         const sp<AoptFile>& file, String8* /* outNewLeafName */)
{
    ...
    png_set_write_fn(write_ptr, (void*)file.get(),
                     png_write_aopt_file, png_flush_aopt_file);
...
status_t postProcessImage(const Bundle* bundle, const sp<AoptAssets>& assets,
                          ResourceTable* table, const sp<AoptFile>& file)
{
    String8 ext(file->getPath().getPathExtension());
