...
{
public:
    static sp<XMLNode> parse(const sp<AoptFile>& file);
    ...
    status_t parseValues(const sp<AoptAssets>& assets, ResourceTable* table);

    status_t assignResourceIds(const sp<AoptAssets>& assets,
                               const ResourceTable* table = NULL);

    status_t flatten(const sp<AoptFile>& dest, bool stripComments,
...
private:
    ...
    status_t flatten_node(const StringPool& strings, const sp<AoptFile>& dest,
            bool stripComments, bool stripRawValues) const;
