...
public:
    ...
    status_t parsePackage(const sp<AoptGroup>& grp);
};
...
ResourceTypeSet::ResourceTypeSet()
    :RefBase(),
     KeyedVector<String8,sp<AoptGroup> >()
{
}
...
public:
    ...
    inline const sp<AoptGroup>& getGroup() const { return mGroup; }
    inline const sp<AoptFile>& getFile() const { return mFile; }
    ...
    const sp<AoptGroup>& grp)
{
    ...
    sp<AoptFile> file = grp->getFiles().valueAt(0);
...
static status_t makeFileResources(Bundle* bundle, const sp<AoptAssets>& assets,
                                  ResourceTable* table,
...
public:
    PreProcessImageWorkUnit(const Bundle* bundle, const sp<AoptAssets>& assets,
            const sp<AoptFile>& file, volatile bool* hasErrors) :
...
private:
    ...
    sp<AoptAssets> mAssets;
    sp<AoptFile> mFile;
    volatile bool* mHasErrors;
};

static status_t preProcessImages(const Bundle* bundle, const sp<AoptAssets>& assets,
...
static void collect_files(const sp<AoptDir>& dir,
        KeyedVector<String8, sp<ResourceTypeSet> >* resources)
{
    const DefaultKeyedVector<String8, sp<AoptGroup> >& groups = dir->getFiles();
    int N = groups.size();
    for (int i=0; i<N; i++) {
        String8 leafName = groups.keyAt(i);
        const sp<AoptGroup>& group = groups.valueAt(i);

        const DefaultKeyedVector<AoptGroupEntry, sp<AoptFile> >& files
                = group->getFiles();
        ...
        if (index < 0) {
            ...
            if (index < 0) {
                ...
                sp<AoptGroup> existingGroup = set->valueAt(index);
                if (kIsDebug) {
...
static void collect_files(const sp<AoptAssets>& ass,
        KeyedVector<String8, sp<ResourceTypeSet> >* resources)
{
    const Vector<sp<AoptDir> >& dirs = ass->resDirs();
    int N = dirs.size();

    for (int i=0; i<N; i++) {
        sp<AoptDir> d = dirs.itemAt(i);
        if (kIsDebug) {
                             ...
                             const char *resType)
{
    ...
    sp<AoptAssets> overlay = assets->getOverlay();
        ...
        if (index >= 0) {
            ...
            for (size_t overlayIndex=0; overlayIndex<overlayCount; overlayIndex++) {
                ...
                if (baseIndex >= 0) {
                    ...
                    // the same flavor.
                    sp<AoptGroup> overlayGroup = overlaySet->valueAt(overlayIndex);
                    sp<AoptGroup> baseGroup = (*baseSet)->valueAt(baseIndex);

                    DefaultKeyedVector<AoptGroupEntry, sp<AoptFile> > overlayFiles =
                            overlayGroup->getFiles();
                    if (bundle->getVerbose()) {
                        DefaultKeyedVector<AoptGroupEntry, sp<AoptFile> > baseFiles =
                                baseGroup->getFiles();
...
static int32_t getPlatformAssetCookie(const AssetManager& assets) {
    // Find the system package (0x01). AOPT always generates attributes
...
static ssize_t extractPlatformBuildVersion(ResXMLTree& tree, Bundle* bundle) {
    ...
    while ((code = tree.next()) != ResXMLTree::END_DOCUMENT && code != ResXMLTree::BAD_DOCUMENT) {
        ...
        String8 error;
        int32_t versionCode = AoptXml::getIntegerAttribute(tree, VERSION_CODE_ATTR, &error);
        if (error != "") {
        ...
        String8 versionName = AoptXml::getAttribute(tree, VERSION_NAME_ATTR, &error);
        if (error != "") {
...
status_t generateAndroidManifestForSplit(Bundle* bundle, const sp<AoptAssets>& assets,
        const sp<ApkSplit>& split, sp<AoptFile>& outFile, ResourceTable* table) {
...
status_t buildResources(Bundle* bundle, const sp<AoptAssets>& assets, sp<ApkBuilder>& builder)
{
    // First, look for a package file to parse.  This is required to
    // be able to generate the resource information.
    sp<AoptGroup> androidManifestFile =
    ...
    /* Only enable UTF-8 if the caller of aopt didn't specifically
    ...
    sp<AoptAssets> current = assets->getOverlay();
    while(current.get()) {
        KeyedVector<String8, sp<ResourceTypeSet> > *resources = 
                new KeyedVector<String8, sp<ResourceTypeSet> >;
    ...
    if (table.hasResources()) {
        ...
        for (size_t i = 0; i < numSplits; i++) {
            ...
            sp<AoptFile> flattenedTable = new AoptFile(String8("resources.arsc"),
                    AoptGroupEntry(), String8());
            ...
            if (split->isBase()) {
                ...
                for (std::set<ConfigDescription>::const_iterator iter = splitConfigs.begin();
                    ...
                    if (AoptConfig::isDensityOnly(config)) {
                ...
                sp<AoptFile> generatedManifest = new AoptFile(String8("AndroidManifest.xml"),
                        AoptGroupEntry(), String8());
    ...
    sp<AoptFile> outManifestFile = new AoptFile(manifestFile->getSourceFile(),
    ...
    const char* schemeIdentChars = "abcdefghijklmnopqrstuvwxyz"
        "ABCDEFGHIJKLMNOPQRSTUVWXYZ._0123456789-";
    ResXMLTree::event_code_t code;
    sp<AoptSymbols> permissionSymbols;
    sp<AoptSymbols> permissionGroupSymbols;
    while ((code=block.next()) != ResXMLTree::END_DOCUMENT
           && code > ResXMLTree::BAD_DOCUMENT) {
        if (code == ResXMLTree::START_TAG) {
            size_t len;
...
static String8 getSymbolPackage(const String8& symbol, const sp<AoptAssets>& assets, bool pub) {
...
static String16 getAttributeComment(const sp<AoptAssets>& assets,
                                    const String8& name,
                                    String16* outTypeComment = NULL)
{
    sp<AoptSymbols> asym = assets->getSymbolsFor(String8("R"));
    if (asym != NULL) {
...
static status_t writeResourceLoadedCallbackForLayoutClasses(
    FILE* fp, const sp<AoptAssets>& assets,
    const sp<AoptSymbols>& symbols, int indent, bool /* includePrivate */)
{
    ...
    for (i=0; i<N; i++) {
        sp<AoptSymbols> nsymbols = symbols->getNestedSymbols().valueAt(i);
...
static status_t writeResourceLoadedCallback(
    FILE* fp, const sp<AoptAssets>& assets, bool includePrivate,
    const sp<AoptSymbols>& symbols, const String8& className, int indent)
{
    size_t i;
    ...
    for (i=0; i<N; i++) {
        const AoptSymbolEntry& sym = symbols->getSymbols().valueAt(i);
        if (sym.typeCode != AoptSymbolEntry::TYPE_INT32) {
            continue;
        }
...
static status_t writeLayoutClasses(
    FILE* fp, const sp<AoptAssets>& assets,
    const sp<AoptSymbols>& symbols, int indent, bool includePrivate, bool nonConstantId)
{
    const char* indentStr = getIndentSpace(indent);
    if (!includePrivate) {
    ...
    for (i=0; i<N; i++) {
        ...
        for (a=0; a<NA; a++) {
            const AoptSymbolEntry& sym(nsymbols->getSymbols().valueAt(a));
            int32_t code = sym.typeCode == AoptSymbolEntry::TYPE_INT32
                    ? sym.int32Val : 0;
            bool isPublic = true;
            if (code == 0) {
...
static status_t writeTextLayoutClasses(
    FILE* fp, const sp<AoptAssets>& assets,
    const sp<AoptSymbols>& symbols, bool includePrivate)
{
    String16 attr16("attr");
    String16 package16(assets->getPackage());
    ...
    bool nonConstantId, bool emitCallback)
{
    ...
    for (i=0; i<N; i++) {
        const AoptSymbolEntry& sym = symbols->getSymbols().valueAt(i);
        if (sym.typeCode != AoptSymbolEntry::TYPE_STRING) {
            continue;
        }
        if (!assets->isJavaSymbol(sym, includePrivate)) {
            continue;
    ...
    sp<AoptSymbols> styleableSymbols;
...
static status_t writeTextSymbolClass(
    FILE* fp, const sp<AoptAssets>& assets, bool includePrivate,
    const sp<AoptSymbols>& symbols, const String8& className)
{
    size_t i;
    status_t err = NO_ERROR;
...
status_t writeResourceSymbols(Bundle* bundle, const sp<AoptAssets>& assets,
    const String8& package, bool includePrivate, bool emitCallback)
{
    ...
    for (size_t i=0; i<N; i++) {
        sp<AoptSymbols> symbols = assets->getSymbols().valueAt(i);
...
status_t
writeProguardForAndroidManifest(ProguardKeepSet* keep, const sp<AoptAssets>& assets, bool mainDex)
{
    status_t err;
    ResXMLTree tree;
    size_t len;
    ...
    while ((code=tree.next()) != ResXMLTree::END_DOCUMENT && code != ResXMLTree::BAD_DOCUMENT) {
        ...
        if (depth == 1) {
            ...
            pkg = AoptXml::getAttribute(tree, NULL, "package");
        } else if (depth == 2) {
            if (tag == "application") {
                inApplication = true;
                keepTag = true;

                String8 agent = AoptXml::getAttribute(tree,
                ...
                if (mainDex) {
                    defaultProcess = AoptXml::getAttribute(tree,
        ...
        if (keepTag) {
            String8 name = AoptXml::getAttribute(tree,
            ...
            if (keepTag && mainDex) {
                String8 componentProcess = AoptXml::getAttribute(tree,
                        "http://schemas.android.com/apk/res/android", "process", &error);
                if (error != "") {
                    fprintf(stderr, "ERROR: %s\n", error.string());
                    return -1;
...
status_t
writeProguardForXml(ProguardKeepSet* keep, const sp<AoptFile>& layoutFile,
...
status_t
writeProguardForLayouts(ProguardKeepSet* keep, const sp<AoptAssets>& assets)
{
    status_t err;
    ...
    const Vector<sp<AoptDir> >& dirs = assets->resDirs();
    const size_t K = dirs.size();
    for (size_t k=0; k<K; k++) {
        const sp<AoptDir>& d = dirs.itemAt(k);
        ...
        const KeyedVector<String8,sp<AoptGroup> > groups = d->getFiles();
        const size_t N = groups.size();
        for (size_t i=0; i<N; i++) {
            const sp<AoptGroup>& group = groups.valueAt(i);
            const DefaultKeyedVector<AoptGroupEntry, sp<AoptFile> >& files = group->getFiles();
...
status_t
writeProguardFile(Bundle* bundle, const sp<AoptAssets>& assets)
{
    status_t err = -1;
...
status_t
writeMainDexProguardFile(Bundle* bundle, const sp<AoptAssets>& assets)
{
    status_t err = -1;
...
status_t
writeDependencyPreReqs(Bundle* /* bundle */, const sp<AoptAssets>& assets, FILE* fp, bool includeRaw)
{
    status_t deps = -1;
