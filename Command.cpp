...
int doVersion(Bundle* bundle)
{
    ...
    printf("Android Overlay Packaging Tool, v0.2-" AOPT_VERSION "\n");
...
{
    Res_value value;
    AoptXml::getResolvedResourceAttribute(resTable, tree, attrRes, &value, outError);
    if (*outError != "") {
        *outError = "error print resolved resource attribute";
        return;
    }
    if (value.dataType == Res_value::TYPE_STRING) {
        String8 result = AoptXml::getResolvedAttribute(resTable, tree, attrRes, outError);
...
static void printCompatibleScreens(ResXMLTree& tree, String8* outError) {
    ...
    while ((code=tree.next()) != ResXMLTree::END_DOCUMENT && code != ResXMLTree::BAD_DOCUMENT) {
        ...
        String8 tag(ctag16);
        if (tag == "screen") {
            int32_t screenSize = AoptXml::getIntegerAttribute(tree,
                    SCREEN_SIZE_ATTR);
            int32_t screenDensity = AoptXml::getIntegerAttribute(tree,
                    SCREEN_DENSITY_ATTR);
    ...
    while ((code=tree.next()) != ResXMLTree::END_DOCUMENT && code != ResXMLTree::BAD_DOCUMENT) {
        if (code == ResXMLTree::END_TAG) {
            ...
            if (depth == 1) {
                ...
                if (tag == "aid-group") {
                    String8 category = AoptXml::getAttribute(tree, CATEGORY_ATTR, &error);
                    if (error != "") {
        ...
        if (strcmp("permissions", option) == 0) {
            ...
            while ((code=tree.next()) != ResXMLTree::END_DOCUMENT &&
                ...
                if (depth == 1) {
                    ...
                    String8 pkg = AoptXml::getAttribute(tree, NULL, "package", NULL);
                    printf("package: %s\n", ResTable::normalizeForOutput(pkg.string()).string());
                } else if (depth == 2) {
                    if (tag == "permission") {
                        String8 error;
                        String8 name = AoptXml::getAttribute(tree, NAME_ATTR, &error);
                        if (error != "") {
                        ...
                        printUsesPermission(name,
                                AoptXml::getIntegerAttribute(tree, REQUIRED_ATTR, 1) == 0,
                                AoptXml::getIntegerAttribute(tree, MAX_SDK_VERSION_ATTR));
            ...
            while ((code=tree.next()) != ResXMLTree::END_DOCUMENT &&
                ...
                if (depth == 1) {
                    ...
                    pkg = AoptXml::getAttribute(tree, NULL, "package", NULL);
                    printf("package: name='%s' ",
                            ResTable::normalizeForOutput(pkg.string()).string());
                    int32_t versionCode = AoptXml::getIntegerAttribute(tree, VERSION_CODE_ATTR,
                            &error);
                    if (error != "") {
                    ...
                    String8 versionName = AoptXml::getResolvedAttribute(res, tree,
                    ...
                    String8 splitName = AoptXml::getAttribute(tree, NULL, "split");
                    if (!splitName.isEmpty()) {
                        printf(" split='%s'", ResTable::normalizeForOutput(
                                    splitName.string()).string());
                    }

                    String8 platformVersionName = AoptXml::getAttribute(tree, NULL,
                            "platformBuildVersionName");
                    printf(" platformBuildVersionName='%s'", platformVersionName.string());
                    printf("\n");

                    int32_t installLocation = AoptXml::getResolvedIntegerAttribute(res, tree,
                    ...
                    if (tag == "application") {
                        ...
                        for (size_t i=0; i<NL; i++) {
                            ...
                            String8 llabel = AoptXml::getResolvedAttribute(res, tree, LABEL_ATTR,
                                    &error);
                            if (llabel != "") {
                        ...
                        for (size_t i=0; i<ND; i++) {
                            ...
                            String8 icon = AoptXml::getResolvedAttribute(res, tree, ICON_ATTR,
                                    &error);
                            if (icon != "") {
                        ...
                        String8 icon = AoptXml::getResolvedAttribute(res, tree, ICON_ATTR, &error);
                        if (error != "") {
                            fprintf(stderr, "ERROR getting 'android:icon' attribute: %s\n",
                                    error.string());
                            goto bail;
                        }
                        int32_t testOnly = AoptXml::getIntegerAttribute(tree, TEST_ONLY_ATTR, 0,
                                &error);
                        if (error != "") {
                        ...
                        String8 banner = AoptXml::getResolvedAttribute(res, tree, BANNER_ATTR,
                                                                       &error);
                        if (error != "") {
                        ...
                        int32_t isGame = AoptXml::getResolvedIntegerAttribute(res, tree,
                                ISGAME_ATTR, 0, &error);
                        if (error != "") {
                        ...
                        int32_t debuggable = AoptXml::getResolvedIntegerAttribute(res, tree,
                        ...
                        int32_t code = AoptXml::getIntegerAttribute(tree, MIN_SDK_VERSION_ATTR,
                                                                    &error);
                        if (error != "") {
                            error = "";
                            String8 name = AoptXml::getResolvedAttribute(res, tree,
                        ...
                        code = AoptXml::getIntegerAttribute(tree, MAX_SDK_VERSION_ATTR);
                        if (code != -1) {
                            printf("maxSdkVersion:'%d'\n", code);
                        }
                        code = AoptXml::getIntegerAttribute(tree, TARGET_SDK_VERSION_ATTR, &error);
                        if (error != "") {
                            error = "";
                            String8 name = AoptXml::getResolvedAttribute(res, tree,
                        ...
                        int32_t reqTouchScreen = AoptXml::getIntegerAttribute(tree,
                                REQ_TOUCH_SCREEN_ATTR, 0);
                        int32_t reqKeyboardType = AoptXml::getIntegerAttribute(tree,
                                REQ_KEYBOARD_TYPE_ATTR, 0);
                        int32_t reqHardKeyboard = AoptXml::getIntegerAttribute(tree,
                                REQ_HARD_KEYBOARD_ATTR, 0);
                        int32_t reqNavigation = AoptXml::getIntegerAttribute(tree,
                                REQ_NAVIGATION_ATTR, 0);
                        int32_t reqFiveWayNav = AoptXml::getIntegerAttribute(tree,
                        ...
                        smallScreen = AoptXml::getIntegerAttribute(tree,
                                SMALL_SCREEN_ATTR, 1);
                        normalScreen = AoptXml::getIntegerAttribute(tree,
                                NORMAL_SCREEN_ATTR, 1);
                        largeScreen = AoptXml::getIntegerAttribute(tree,
                                LARGE_SCREEN_ATTR, 1);
                        xlargeScreen = AoptXml::getIntegerAttribute(tree,
                                XLARGE_SCREEN_ATTR, 1);
                        anyDensity = AoptXml::getIntegerAttribute(tree,
                                ANY_DENSITY_ATTR, 1);
                        requiresSmallestWidthDp = AoptXml::getIntegerAttribute(tree,
                                REQUIRES_SMALLEST_WIDTH_DP_ATTR, 0);
                        compatibleWidthLimitDp = AoptXml::getIntegerAttribute(tree,
                                COMPATIBLE_WIDTH_LIMIT_DP_ATTR, 0);
                        largestWidthLimitDp = AoptXml::getIntegerAttribute(tree,
                                LARGEST_WIDTH_LIMIT_DP_ATTR, 0);
                    } else if (tag == "feature-group") {
                        withinFeatureGroup = true;
                        FeatureGroup group;
                        group.label = AoptXml::getResolvedAttribute(res, tree, LABEL_ATTR, &error);
                        if (error != "") {
                        ...
                        if (name != "" && error == "") {
                            ...
                            int32_t req = AoptXml::getIntegerAttribute(tree, REQUIRED_ATTR, 1,
                                                                       &error);
                            if (error != "") {
                            ...
                            int32_t version = AoptXml::getIntegerAttribute(tree, androidSchema,
                                                                           "version", 0, &error);
                            if (error != "") {
                                SourcePos(manifestFile, tree.getLineNumber()).error(
                            ...
                            int vers = AoptXml::getIntegerAttribute(tree,
                        ...
                        printUsesPermissionSdk23(
                                name, AoptXml::getIntegerAttribute(tree, MAX_SDK_VERSION_ATTR));
                        ...
                        String8 name = AoptXml::getAttribute(tree, NAME_ATTR, &error);
                        if (name != "" && error == "") {
                            String8 publicKey = AoptXml::getAttribute(tree, PUBLIC_KEY_ATTR,
                                                                      &error);
                            if (publicKey != "" && error == "") {
                    ...
                    if (withinApplication) {
                        if(tag == "activity") {
                            withinActivity = true;
                            activityName = AoptXml::getAttribute(tree, NAME_ATTR, &error);
                            if (error != "") {
                                fprintf(stderr, "ERROR getting 'android:name' attribute: %s\n",
                                        error.string());
                                goto bail;
                            ...
                            activityLabel = AoptXml::getResolvedAttribute(res, tree, LABEL_ATTR,
                                    &error);
                            if (error != "") {
                            ...
                            activityIcon = AoptXml::getResolvedAttribute(res, tree, ICON_ATTR,
                                    &error);
                            if (error != "") {
                                fprintf(stderr, "ERROR getting 'android:icon' attribute: %s\n",
                                        error.string());
                            ...
                            activityBanner = AoptXml::getResolvedAttribute(res, tree, BANNER_ATTR,
                                    &error);
                            if (error != "") {
                                fprintf(stderr, "ERROR getting 'android:banner' attribute: %s\n",
                                        error.string());
                            ...
                            int32_t orien = AoptXml::getResolvedIntegerAttribute(res, tree,
                            ...
                            String8 libraryName = AoptXml::getAttribute(tree, NAME_ATTR, &error);
                            if (error != "") {
                                fprintf(stderr,
                            ...
                            int req = AoptXml::getIntegerAttribute(tree,
                                    REQUIRED_ATTR, 1);
                            ...
                            withinReceiver = true;
                            receiverName = AoptXml::getAttribute(tree, NAME_ATTR, &error);
                            ...
                            String8 permission = AoptXml::getAttribute(tree, PERMISSION_ATTR,
                                    &error);
                            if (error == "") {
                            ...
                            withinService = true;
                            serviceName = AoptXml::getAttribute(tree, NAME_ATTR, &error);
                            ...
                            bool exported = AoptXml::getResolvedIntegerAttribute(res, tree,
                                    EXPORTED_ATTR, &error);
                            if (error != "") {
                                fprintf(stderr,
                            ...
                            bool grantUriPermissions = AoptXml::getResolvedIntegerAttribute(
                            ...
                            String8 permission = AoptXml::getResolvedAttribute(res, tree,
                            ...
                            String8 metaDataName = AoptXml::getResolvedAttribute(res, tree,
                                    NAME_ATTR, &error);
                            if (error != "") {
                                fprintf(stderr, "ERROR getting 'android:name' attribute for "
                                        "meta-data:%s\n", error.string());
                        ...
                        String8 name = AoptXml::getResolvedAttribute(res, tree, NAME_ATTR, &error);
                        if (name != "" && error == "") {
                            Feature feature(true);

                            int32_t featureVers = AoptXml::getIntegerAttribute(
                            ...
                            int vers = AoptXml::getIntegerAttribute(tree, GL_ES_VERSION_ATTR,
                                    &error);
                            if (error == "") {
                    ...
                    if (tag == "intent-filter") {
                        ...
                        if (name == "android.nfc.cardemulation.host_apdu_service" ||
                            ...
                            String8 xmlPath = AoptXml::getResolvedAttribute(res, tree,
                                    RESOURCE_ATTR, &error);
                            if (error != "") {
                    ...
                    String8 action;
                    if (tag == "action") {
                        action = AoptXml::getAttribute(tree, NAME_ATTR, &error);
                        if (error != "") {
                            fprintf(stderr, "ERROR getting 'android:name' attribute: %s\n",
                                    error.string());
                            goto bail;
                    ...
                    if (tag == "category") {
                        String8 category = AoptXml::getAttribute(tree, NAME_ATTR, &error);
                        if (error != "") {
...
static status_t addResourcesToBuilder(const sp<AoptDir>& dir, const sp<ApkBuilder>& builder, bool ignoreConfig=false) {
    const size_t numDirs = dir->getDirs().size();
    for (size_t i = 0; i < numDirs; i++) {
        bool ignore = ignoreConfig;
        const sp<AoptDir>& subDir = dir->getDirs().valueAt(i);
    ...
    for (size_t i = 0; i < numFiles; i++) {
        sp<AoptGroup> gp = dir->getFiles().valueAt(i);
...
int doPackage(Bundle* bundle)
{
    ...
    if (bundle->getSplitConfigurations().size() > 0) {
        ...
        for (size_t i = 0; i < numSplits; i++) {
            ...
            if (!AoptConfig::parseCommaSeparatedList(splitStrs[i], &configs)) {
