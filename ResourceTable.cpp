    ...
    while ((code=block.next()) != ResXMLTree::END_DOCUMENT && code != ResXMLTree::BAD_DOCUMENT) {
        if (code == ResXMLTree::START_TAG) {
            ...
            if (strcmp16(block.getElementName(&len), skip16.string()) == 0) {
                ...
                sp<AoptSymbols> symbols = assets->getJavaSymbolsFor(String8("R"));
                if (symbols != NULL) {
                    symbols = symbols->addNestedSymbol(String8(type), srcPos);
                }
                if (symbols != NULL) {
                ...
                if (!localHasErrors) {
                    ...
                    sp<AoptSymbols> styleSymbols = symbols;
                    if (symbols != NULL) {
...
status_t ResourceTable::addSymbols(const sp<AoptSymbols>& outSymbols,
        ...
        const bool isBase)
{
    ...
    Vector<sp<AoptFile> > flatPackages;
    for (pi=0; pi<N; pi++) {
        sp<Package> p = mOrderedPackages.itemAt(pi);
        if (p->getTypes().size() == 0) {
            // Empty, skip!
        ...
        sp<AoptFile> strFile = p->getTypeStringsData();
...
status_t ResourceTable::flattenLibraryTable(const sp<AoptFile>& dest, const Vector<sp<Package> >& libs) {
...
ssize_t ResourceTable::Entry::flatten(Bundle* /* bundle */, const sp<AoptFile>& data, bool isPublic,
        bool isOverlay)
{
    size_t amt = 0;
...
status_t ResourceTable::Package::setKeyStrings(const sp<AoptFile>& data)
{
...
status_t ResourceTable::Package::setStrings(const sp<AoptFile>& data,
 ...
 *
 * AOPT has traditionally assigned resource IDs to public attributes,
                ...
                for (size_t ei = 0; ei < entryCount; ei++) {
                    ...
                    for (size_t bi = 0; bi < bagCount; bi++) {
                        ...
                        if (sdkLevel > 1 && sdkLevel > config.sdkVersion && sdkLevel > minSdk) {
                            AoptUtil::appendValue(attributesToRemove, sdkLevel, bag.keyAt(bi));
                        }
                    }
            ...
            for (size_t c = 0; c < configCount; c++) {
                ...
                for (size_t ce = 0; ce < configEntryCount; ce++) {
                    ...
                    if (AoptConfig::isDensityOnly(config)) {
                        ...
                        AoptUtil::appendValue(resources, symbol,
...
status_t ResourceTable::processBundleFormatImpl(const Bundle* bundle,
                                                ...
                                                Vector<sp<XMLNode> >* namespaces) {
    const String16 kAoptNamespaceUri16("http://schemas.android.com/aapt");
    ...
    for (size_t i = 0; i < children.size(); i++) {
        ...
        if (child->getElementNamespace() != kAoptNamespaceUri16 ||
        ...
        workItem.file = new AoptFile(target->getSourceFile(), target->getGroupEntry(),
