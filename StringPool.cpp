...
sp<AoptFile> StringPool::createStringBlock()
{
    sp<AoptFile> pool = new AoptFile(String8(), AoptGroupEntry(),
                                     String8());
...
status_t StringPool::writeStringBlock(const sp<AoptFile>& pool)
{
    // Allow appending.  Sorry this is a little wacky.
    if (pool->getSize() > 0) {
        sp<AoptFile> block = createStringBlock();
        if (block == NULL) {
