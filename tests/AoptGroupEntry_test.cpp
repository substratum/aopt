...
static ::testing::AssertionResult TestParse(AoptGroupEntry& entry, const String8& dirName,
        String8* outType) {
...
static ::testing::AssertionResult TestParse(AoptGroupEntry& entry, const char* input,
        String8* outType) {
    return TestParse(entry, String8(input), outType);
}

TEST(AoptGroupEntryTest, ParseNoQualifier) {
    AoptGroupEntry entry;
    String8 type;
    EXPECT_TRUE(TestParse(entry, "menu", &type));
    EXPECT_EQ(String8("menu"), type);
}

TEST(AoptGroupEntryTest, ParseCorrectType) {
    AoptGroupEntry entry;
    String8 type;
