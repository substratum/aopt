...
static bool isHidden(const char *root, const char *path)
{
    ...
    if (ignore && chatty) {
        fprintf(stderr, "    (skipping %s '%s' due to ANDROID_AOPT_IGNORE pattern '%s')\n",
...
void AoptLocaleValue::setLanguage(const char* languageChars) {
     size_t i = 0;
...
void AoptLocaleValue::setRegion(const char* regionChars) {
    size_t i = 0;
...
void AoptLocaleValue::setScript(const char* scriptChars) {
    size_t i = 0;
...
void AoptLocaleValue::setVariant(const char* variantChars) {
     size_t i = 0;
...
bool AoptLocaleValue::initFromFilterString(const String8& str) {
     // A locale (as specified in the filter) is an underscore separated name such
     // as "en_US", "en_Latn_US", or "en_US_POSIX".
     Vector<String8> parts = AoptUtil::splitAndLowerCase(str, '_');
...
int AoptLocaleValue::initFromDirName(const Vector<String8>& parts, const int startIndex) {
    ...
    if (part[0] == 'b' && part[1] == '+') {
        ...
        Vector<String8> subtags = AoptUtil::splitAndLowerCase(part, '+');
...
void AoptLocaleValue::initFromResTable(const ResTable_config& config) {
...
void AoptLocaleValue::writeTo(ResTable_config* out) const {
...
bool
AoptGroupEntry::initFromDirName(const char* dir, String8* resType)
{
    ...
    if (q != NULL) {
        if (!AoptConfig::parse(String8(q + 1), &mParams)) {
            return false;
        }
    }
...
String8
AoptGroupEntry::toDirName(const String8& resType) const
{
    String8 s = resType;
...
void* AoptFile::editData(size_t size)
{
    if (size <= mBufferSize) {
        mDataSize = size;
        return mData;
...
void* AoptFile::editDataInRange(size_t offset, size_t size)
{
    return (void*)(((uint8_t*) editData(offset + size)) + offset);
}

void* AoptFile::editData(size_t* outSize)
{
    if (outSize) {
        *outSize = mDataSize;
    }
...
void* AoptFile::padData(size_t wordSize)
{
...
status_t AoptFile::writeData(const void* data, size_t size)
{
...
String8 AoptFile::getPrintableSource() const
{
    if (hasData()) {
...
status_t AoptGroup::addFile(const sp<AoptFile>& file, const bool overwriteDuplicate)
{
    ...
    AoptConfig::applyVersionForCompatibility(&withoutVersion);

    const sp<AoptFile>& originalFile = mFiles.valueAt(index);
...
void AoptGroup::removeFile(size_t index)
{
	mFiles.removeItemsAt(index);
}

void AoptGroup::print(const String8& prefix) const
{
    printf("%s%s\n", prefix.string(), getPath().string());
    const size_t N=mFiles.size();
    size_t i;
    for (i=0; i<N; i++) {
        sp<AoptFile> file = mFiles.valueAt(i);
        const AoptGroupEntry& e = file->getGroupEntry();
        if (file->hasData()) {
...
String8 AoptGroup::getPrintableSource() const
{
    if (mFiles.size() > 0) {
...
status_t AoptDir::addFile(const String8& name, const sp<AoptGroup>& file)
{
...
status_t AoptDir::addDir(const String8& name, const sp<AoptDir>& dir)
{
...
sp<AoptDir> AoptDir::makeDir(const String8& path)
{
    String8 name;
    String8 remain = path;

    sp<AoptDir> subdir = this;
    ...
    sp<AoptDir> dir = new AoptDir(name, subdir->mPath.appendPathCopy(name));
    subdir->mDirs.add(name, dir);
    return dir;
}

void AoptDir::removeFile(const String8& name)
{
    mFiles.removeItem(name);
}

void AoptDir::removeDir(const String8& name)
{
    mDirs.removeItem(name);
}

status_t AoptDir::addLeafFile(const String8& leafName, const sp<AoptFile>& file,
        const bool overwrite)
{
    ...
    if (mFiles.indexOfKey(leafName) >= 0) {
        ...
        group = new AoptGroup(leafName, mPath.appendPathCopy(leafName));
...
ssize_t AoptDir::slurpFullTree(Bundle* bundle, const String8& srcDir,
                            const AoptGroupEntry& kind, const String8& resType,
    ...
    for (i = 0; i < N; i++) {
        ...
        if (type == kFileTypeDirectory) {
            ...
            if (mDirs.indexOfKey(fileNames[i]) >= 0) {
                ...
                subdir = new AoptDir(fileNames[i], mPath.appendPathCopy(fileNames[i]));
                notAdded = true;
            }
            ...
            sp<AoptFile> file = new AoptFile(pathName, kind, resType);
...
status_t AoptDir::validate() const
{
...
void AoptDir::print(const String8& prefix) const
{
...
String8 AoptDir::getPrintableSource() const
{
    if (mFiles.size() > 0) {
...
status_t AoptSymbols::applyJavaSymbols(const sp<AoptSymbols>& javaSymbols)
{
    status_t err = NO_ERROR;
    size_t N = javaSymbols->mSymbols.size();
    for (size_t i=0; i<N; i++) {
        const String8& name = javaSymbols->mSymbols.keyAt(i);
        const AoptSymbolEntry& entry = javaSymbols->mSymbols.valueAt(i);
    ...
    for (size_t i=0; i<N; i++) {
        ...
        const sp<AoptSymbols>& symbols = javaSymbols->mNestedSymbols.valueAt(i);
...
AoptAssets::AoptAssets()
    : AoptDir(String8(), String8()),
      mHavePrivateSymbols(false),
      mChanged(false), mHaveIncludedAssets(false),
      mRes(NULL) {}

const SortedVector<AoptGroupEntry>& AoptAssets::getGroupEntries() const {
    if (mChanged) {
    }
    return mGroupEntries;
}

status_t AoptAssets::addFile(const String8& name, const sp<AoptGroup>& file)
{
    mChanged = true;
    return AoptDir::addFile(name, file);
}

sp<AoptFile> AoptAssets::addFile(
        const String8& filePath, const AoptGroupEntry& entry,
        const String8& srcDir, sp<AoptGroup>* outGroup,
    ...
    while (remain.length() > 0) {
        ...
        if (remain.length() == 0) {
            ...
            if (i >= 0) {
                ...
                group = new AoptGroup(rootStr, filePath);
                status_t res = dir->addFile(rootStr, group);
                if (res != NO_ERROR) {
                    return NULL;
                }
            }
            file = new AoptFile(srcDir.appendPathCopy(filePath), entry, resType);
            ...
            if (i >= 0) {
                ...
                sp<AoptDir> subdir = new AoptDir(rootStr, partialPath);
...
void AoptAssets::addResource(const String8& leafName, const String8& path,
                const sp<AoptFile>& file, const String8& resType)
{
    sp<AoptDir> res = AoptDir::makeDir(kResString);
    String8 dirname = file->getGroupEntry().toDirName(resType);
    sp<AoptDir> subdir = res->makeDir(dirname);
    sp<AoptGroup> grr = new AoptGroup(leafName, path);
    grr->addFile(file);
...
ssize_t AoptAssets::slurpFromArgs(Bundle* bundle)
{
    int count;
    int totalCount = 0;
    FileType type;
    ...
    if (bundle->getAndroidManifestFile() != NULL) {
        ...
        addFile(srcFile.getPathLeaf(), AoptGroupEntry(), srcFile.getPathDir(),
                NULL, String8());
        totalCount++;
    }
    ...
    for (int i = 0; i < AN; i++) {
        ...
        sp<AoptDir> assetAoptDir = makeDir(String8(kAssetDir));
        AoptGroupEntry group;
        count = assetAoptDir->slurpFullTree(bundle, assetRoot, group,
    ...
    for (size_t i=0; i<dirCount; i++) {
        ...
        if (res) {
            ...
            if (type == kFileTypeDirectory) {
                if (i>0) {
                    sp<AoptAssets> nextOverlay = new AoptAssets();
    ...
    for (int arg=0; arg<N; arg++) {
         ...
         */
        count = slurpFullTree(bundle, assetRoot, AoptGroupEntry(), String8(), mFullAssetPaths);
        if (count < 0) {
...
ssize_t AoptAssets::slurpFullTree(Bundle* bundle, const String8& srcDir,
                                    const AoptGroupEntry& kind,
                                    const String8& resType,
                                    sp<FilePathStore>& fullResPaths,
                                    const bool overwrite)
{
    ssize_t res = AoptDir::slurpFullTree(bundle, srcDir, kind, resType, fullResPaths, overwrite);
    if (res > 0) {
...
ssize_t AoptAssets::slurpResourceTree(Bundle* bundle, const String8& srcDir)
{
    ssize_t err = 0;
    ...
    while (1) {
        ...
        if (type == kFileTypeDirectory) {
            sp<AoptDir> dir = makeDir(resType);
            ...
            // for all configs.
            sp<AoptDir> rdir = resDir(resType);
            if (rdir == NULL) {
                mResDirs.add(dir);
            }
        } else {
...
ssize_t
AoptAssets::slurpResourceZip(Bundle* /* bundle */, const char* filename)
{
    int count = 0;
    SortedVector<AoptGroupEntry> entries;
    ...
    for (int i=0; i<N; i++) {
        ...
        sp<AoptDir> dir = dirName == "" ? this : makeDir(dirName);
        ...
        if (entryName.walkPath(&remain) == kResourceDir) {
            ...
            // these are untyped and don't have an AoptGroupEntry
        }
        ...
        sp<AoptFile> file = new AoptFile(entryName, kind, resType);
...
status_t AoptAssets::filter(Bundle* bundle)
{
    ...
    if (bundle->getPreferredDensity().size() > 0) {
        ...
        if (!AoptConfig::parseDensity(bundle->getPreferredDensity().string(), &preferredConfig)) {
    ...
    const Vector<sp<AoptDir> >& resdirs = mResDirs;
    const size_t ND = resdirs.size();
    for (size_t i=0; i<ND; i++) {
        const sp<AoptDir>& dir = resdirs.itemAt(i);
            ...
            for (size_t k=0; k<grp->getFiles().size(); k++) {
                ...
                if (config.density != 0 && config.density != preferredDensity) {
                    ...
                    for (size_t m=0; m<grp->getFiles().size(); m++) {
                        ...
                        sp<AoptFile> mfile = grp->getFiles().valueAt(m);
                        const ResTable_config& mconfig(mfile->getGroupEntry().toParams());
                        if (AoptConfig::isSameExcept(config, mconfig, ResTable_config::CONFIG_DENSITY)) {
...
sp<AoptSymbols> AoptAssets::getSymbolsFor(const String8& name)
{
    sp<AoptSymbols> sym = mSymbols.valueFor(name);
    if (sym == NULL) {
        sym = new AoptSymbols();
...
sp<AoptSymbols> AoptAssets::getJavaSymbolsFor(const String8& name)
{
    sp<AoptSymbols> sym = mJavaSymbols.valueFor(name);
    if (sym == NULL) {
        sym = new AoptSymbols();
...
status_t AoptAssets::applyJavaSymbols()
{
    size_t N = mJavaSymbols.size();
    for (size_t i=0; i<N; i++) {
        const String8& name = mJavaSymbols.keyAt(i);
        const sp<AoptSymbols>& symbols = mJavaSymbols.valueAt(i);
        ssize_t pos = mSymbols.indexOfKey(name);
        if (pos < 0) {
            SourcePos pos;
            pos.error("Java symbol dir %s not defined\n", name.string());
...
bool AoptAssets::isJavaSymbol(const AoptSymbolEntry& sym, bool includePrivate) const {
...
status_t AoptAssets::buildIncludedResources(Bundle* bundle)
{
...
status_t AoptAssets::addIncludedResources(const sp<AoptFile>& file)
{
...
const ResTable& AoptAssets::getIncludedResources() const
{
    return mIncludedAssets.getResources(false);
}

AssetManager& AoptAssets::getAssetManager()
{
    return mIncludedAssets;
}

void AoptAssets::print(const String8& prefix) const
{
    ...
    printf("\n%sFiles:\n", prefix.string());
    AoptDir::print(innerPrefix);

    printf("\n%sResource Dirs:\n", prefix.string());
    const Vector<sp<AoptDir> >& resdirs = mResDirs;
    const size_t NR = resdirs.size();
    for (size_t i=0; i<NR; i++) {
        const sp<AoptDir>& d = resdirs.itemAt(i);
        printf("%s  Type %s\n", prefix.string(), d->getLeaf().string());
        d->print(innerInnerPrefix);
    }
}

sp<AoptDir> AoptAssets::resDir(const String8& name) const
{
    const Vector<sp<AoptDir> >& resdirs = mResDirs;
