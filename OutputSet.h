...
public:
    OutputEntry() {}
    OutputEntry(const android::String8& path, const android::sp<const AoptFile>& file)
        : mPath(path), mFile(file) {}

    inline const android::sp<const AoptFile>& getFile() const {
        return mFile;
    }
...
private:
    ...
    android::sp<const AoptFile> mFile;
};
